...
Select * from Stg_Billing where PatientID NOT IN (select PatientID from Stg_Patient);
select * from Stg_MedicalProcedure s where ProcedureName IS NULL;
Select ProcedureID, count(*) from Stg_MedicalProcedure group by ProcedureID having COUNT(*) > 1;